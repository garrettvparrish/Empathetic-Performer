...
@property NSMutableArray *noteHistory;
...
- (void)awakeFromNib {
    ...
    MIDIClientCreate(CFSTR("Creative Companion"), NULL, NULL, &client);
    ...
    NSString *keyboard1Name = @"USB O2";
    NSString *keyboard2Name = @"Keystation Pro 88";
    MIDIDeviceRef keyboard1 = 0;
    MIDIDeviceRef keyboard2 = 1;
    ...
    for (int i = 0; i < numberOfDevices; i++) {
        ...
        if (device) {
            ...
            if (MIDIObjectGetStringProperty(device, kMIDIPropertyName, &name) == noErr) {
                ...
                if ([keyboard1Name isEqualToString:deviceName]) {
                    keyboard1 = device;
                    CFRelease(name);
                } else if ([keyboard2Name isEqualToString:deviceName]) {
                    keyboard2 = device;
                    CFRelease(name);
                }
            }
    ...
    // Connect the port
    MIDIEndpointRef entity1 = MIDIDeviceGetEntity(keyboard1, 0);
    MIDIEndpointRef source1 = MIDIEntityGetSource(entity1, 0);

    MIDIEndpointRef entity2 = MIDIDeviceGetEntity(keyboard2, 1);
    MIDIEndpointRef source2 = MIDIEntityGetSource(entity2, 1);

    MIDIPortConnectSource(iPort, source1, NULL);
    MIDIPortConnectSource(iPort, source2, NULL);
    identity = self;
...
void MIDIInput (const MIDIPacketList *packets, void *source, void *connectSource) {
    ...
    int mode, note, velocity;
    // mode
    mode = (int)packets->packet[0].data[0];
    // note number
    note = (int)packets->packet[0].data[1];
    // Velocity
    velocity = (int)packets->packet[0].data[2];
    
    NSLog(@"%i, %i, %i", mode, note, velocity);
    [identity handleNote:note];
}
...
- (void) handleNote:(int) note {
    ...
    for (int i = 0; i < buffer; i++) {
        ...
        if (note == history[i]) {
            if (currentHeat < 180) {
                float rndValue = (((float)arc4random()/0x100000000));
                if (rndValue < .30) {
                    currentHeat++;
                    NSLog(@"INCREASING");
                }
    ...
    history[buffer-1] = note;
    
    [self writeString:[NSString stringWithFormat:@"%i$",currentHeat]];
}
...
///THIS IS WHAT WE WANT TO SEND THE VALUE OF STUFF //

// send a string to the serial port
- (void) writeString: (NSString *) str {
    NSLog(@"Writing String:%@", str);
	if(serialFileDescriptor!=-1) {
